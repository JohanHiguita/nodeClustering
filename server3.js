...
if (cluster.isMaster) {
    ...
    established that allows sending messages between parent and child. */
    ...
    cluster.on("online", worker => {
        ...
        worker.on("message", num => {
            console.log(`Message received: num${num}`);
            if (num % 2 === 0) {
                worker1.send(num); 
            }
            else {
    ...
    app.get("/", (request, response) => {
        //this sends a message to the primary(master). It is identical to process.send()
